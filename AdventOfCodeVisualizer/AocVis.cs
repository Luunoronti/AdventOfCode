...
namespace AdventOfCodeVisualizer;
    ...
    private static void ThreadFunc()
    {
        ...
        while (true)
        {
            ...
            while (listener.Pending())
            {
                ...
                clients.Add(new ClientConn { tcp = client, stream = client.GetStream(), expected = 0, queue = new() });
            }
            ...
            for (var i = 0; i < clients.Count; i++)
            {
                ...
                if (avail > 0)
                {
                    var read = client.stream.Read(input);
                    client.queue.Enqueue(input, 0, read);
                    if (client.expected == 0)
                        client.expected = BitConverter.ToInt32(input);
                    ...
                    while (client.expected != 0 && client.expected >= client.queue.Length)
                    {
                        ...
                        App.MainWindow.DispatcherQueue.TryEnqueue(() => { App.DispatchMessage(data); });

                        if (client.queue.Length >= 4)
                        {
                            var q = client.queue;
                            var d = new byte[4] { q.PeekOne(0), q.PeekOne(1), q.PeekOne(2), q.PeekOne(3) };
                            client.expected = BitConverter.ToInt32(d);
                        }
                        else
                        {
                            client.expected = 0;
