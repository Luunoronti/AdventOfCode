...
void View::DrawPlayback()
{
    ...
    ImGui::SameLine();
    ImGui::Checkbox( "Zoom", &m_playback.zoom );
    ImGui::SliderInt( "Zoom Level", &m_playback.zoomLevel, 1, 20 );
     ...
     if( m_playback.zoom )
    {
        if( fi->flip )
        {
            ImGui::Image( m_playback.texture, ImVec2( fi->w * m_playback.zoomLevel * scale, fi->h * m_playback.zoomLevel * scale ), ImVec2( 0, 1 ), ImVec2( 1, 0 ) );
        }
        else
        {
            ImGui::Image( m_playback.texture, ImVec2( fi->w * m_playback.zoomLevel * scale, fi->h * m_playback.zoomLevel * scale ) );
        }
    }
    else
    {
