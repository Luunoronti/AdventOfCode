    ...
    if( ImGui::IsItemHovered() )
    {
        ...
        if( m_showMessageImages )
        {
            ...
            if( fi )
            {
                ...
                if( fi->flip )
                {
                    ImGui::Image( m_frameTexture, ImVec2( fi->w * frameImageScale, fi->h  * frameImageScale), ImVec2( 0, 1 ), ImVec2( 1, 0 ) );
                }
                else
                {
                    ImGui::Image( m_frameTexture, ImVec2( fi->w * frameImageScale, fi->h * frameImageScale ) );
                }
                ImGui::EndTooltip();
            }
        }
